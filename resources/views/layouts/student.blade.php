...
<body class="bg-gray-100 flex h-screen overflow-hidden font-sans" x-data="{ sidebarOpen: false }">
    ...
    {{-- Mobile Overlay --}}
    <div x-show="sidebarOpen" @click="sidebarOpen = false" x-transition:enter="transition-opacity ease-linear duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition-opacity ease-linear duration-300" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed inset-0 bg-gray-900/50 z-20 lg:hidden backdrop-blur-sm" style="display: none;"></div>
    ...
    <main class="flex-1 min-w-0 overflow-y-auto flex flex-col bg-gray-100">
        ...
        <header
            class="px-8 lg:px-16 bg-white border-b border-gray-200 py-4 flex justify-between items-center sticky top-0 z-10 shrink-0">
            ...
            <div class="relative" x-data="{ open: false }">
                <button @click="open = !open" @click.outside="open = false"
                    ...
                    <span
                        class="text-sm text-gray-500 hidden sm:block group-hover:text-gray-700 transition">Student</span>
                    <div
                        class="h-8 w-8 rounded-full bg-indigo-600 flex items-center justify-center text-white font-bold shadow-sm overflow-hidden border border-indigo-200">
